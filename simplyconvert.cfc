...
component displayname="SimplyConvertCFC" {
  ...
  public any function init(
    ...
    var secrets = {
      "apiKey"      : "SIMPLYCONVERT_API_KEY",
      "company_uuid": "SIMPLYCONVERT_COMPANY_UUID"
    };
      ...
      if( isJsonPayload(headers) ){
        ...
        for( var param in payload ){
          if( !variables.fileFields.contains(param) ){
            var payload_value = isSimpleValue(payload[param]) ? payload[param] : serializeJSON(payload[param]);
